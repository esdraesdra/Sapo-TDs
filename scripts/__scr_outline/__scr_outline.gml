...
if (shaders_are_supported()) {
	if (shader_is_compiled(__shd_outline)) {
		global.__OL_cache.outline_compatible = true;
	} else {
		show_debug_message(@"
=====================================================
Outline System Error: __shd_outline could not be compiled on this hardware!
=====================================================
")
	}
} else {
...
function __outline_start() {
	if (global.__OL_cache.outline_compatible) {
...
function __outline_surface_create(_wid, _hei) {
    ...
    array_push(global.__OL_cache.surface_mng.structs, weak_ref_create(_struct));
    
    var _meta = {surf: _surf};
    array_push(global.__OL_cache.surface_mng.meta, _meta);
...
function __outline_gc() {
	...
	var _mng = global.__OL_cache.surface_mng;
...
function ol_config(
	_thick	= global.__OL_cache.config.line_width, 
	_col	= global.__OL_cache.config.line_col, 
	_alpha	= global.__OL_cache.config.line_alpha, 
	_round	= global.__OL_cache.config.roundness, 
	_tol	= global.__OL_cache.config.tolerance, 
	_res	= global.__OL_cache.config.resolution, 
	_uv_bnd = global.__OL_cache.config.uv_bound_mode) {
